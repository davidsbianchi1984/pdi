...
| **Overlay** — *40 Audit · overlay*, and every desktop view | a pinned strip with the three counts and a way in | an agent that reports only on its own screen is one somebody has to remember to check. On desktop it rides on **every** view, because a console is watched from, not visited |
...
The full capability set is **also rendered in the desktop frame**: every one of the 40 mobile screens below has a wide operator-console counterpart in [`docs/screens/desktop/`](docs/screens/desktop) — window chrome, left nav rail, right status rail — generated by the same `docs/screens/build.py`.
...
| `POST`/`GET /tenants/{id}/baa`, `DELETE …/baa`, `GET /baa` | admin / tenant | **BAA enforcement**: the operator records each customer's executed Business Associate Agreement ([template](docs/baa-template.md)); HIPAA-program transfers/intakes are refused (403) for tenants without one on file; tenants check their own standing at `GET /baa`; execution/termination land in the audit chain |
