...
def set_language(tenant_id: str, language: str, mode: str = "pre") -> str:
    ...
    if mode not in MODES:
        raise ValueError(fill(MUST_BE_ONE_OF, field="mode", choices=MODES))
    from . import db
    conn = db.connect()
    conn.execute(
...
#: keep their exact English; the sentences JIM and QRME already say are
#: carried here with the same frames, so the trio refuses in one voice.
BEACON_NEEDS_TARGET = "a {kind} beacon needs a {what} to point at"
OFFLINE_UNREACHABLE_HINT = ("offline mode is on, so {label} cannot reach {host}. Nothing leaves this machine while PDI_OFFLINE is set — point it at a host on this network, or turn offline mode off.")
OFFLINE_UNREACHABLE = ("offline mode is on, so {label} cannot reach {host}. Nothing leaves this machine while PDI_OFFLINE is set.")
OLD_KEY_SEALS_KEYRING = ("the old key does not open key version {version}, so this rotation would seal the keyring shut — nothing was changed")
RETENTION_ONE_OF = ("retention must be one of {choices} or a positive day count")
UNKNOWN_COMPLIANCE_PROGRAMS = "unknown compliance program(s): {got}"
NOT_A_TIMEZONE = ("{got} is not a timezone this system knows — use an IANA name like 'Europe/Lisbon'")
OP_IDENTIFIER_SEGMENT = ("'op' contains a segment that looks like an identifier ({got}) — the client's redaction has stopped working, and accepting this would hide that")
...
UNKNOWN_KEYS = ("unknown top-level keys {keys} — this intake accepts exactly {accepts}. A new key is refused rather than ignored: silently dropping it would let a client start sending content and never learn that nobody wanted it.")
...
SHORT_PLAIN_STRING = ("{field} must be a short plain string (≤{max} chars, no newlines or punctuation beyond ._-()/;:+) — got {got}")

#: The two sentences the widened sweep surfaced in the resident, after
#: `i18n.raised` started counting as stringifying.
VAULT_NO_EARS = "the vault has no ears for this: {detail}"
FETCH_READS_PAGES = ("{tool} reads pages, and that url names a recording — fetch.listen is the door that hears it")

#: Derived from the table below rather than repeated.
TEMPLATES = (MUST_BE_ONE_OF, VAULT_NO_EARS, FETCH_READS_PAGES,
             BEACON_NEEDS_TARGET, OFFLINE_UNREACHABLE_HINT,
             OFFLINE_UNREACHABLE, OLD_KEY_SEALS_KEYRING, RETENTION_ONE_OF,
             UNKNOWN_COMPLIANCE_PROGRAMS, NOT_A_TIMEZONE,
             OP_IDENTIFIER_SEGMENT, OP_SHAPE, MAX_PROBLEMS_PER_REPORT,
             COULD_NOT_FETCH, MISSING_KEYS, NO_SUCH_FACE, NO_SUCH_STEP,
             PROBLEM_MISSING_KEYS, PANE_BOTTOM_CORNER, UNKNOWN_CONNECTOR,
             UNKNOWN_DAY_RANGE, UNKNOWN_DAY, UNKNOWN_MODE_ONE_OF,
             UNKNOWN_PROBLEM_KEYS, UNKNOWN_STATE, UNKNOWN_KEYS,
             UNREADABLE_TIME, NOTHING_READABLE, APP_DOES_NOT_OFFER,
             NO_COLLECT_SUPPORT, FACE_NOT_CARRIED, UNKNOWN_CHOICE,
             UNKNOWN_LANGUAGE, UNKNOWN_ROBOT_MODEL_QUOTED,
             CONNECTOR_NOT_GRANTED_Q, NO_SUCH_THING, SHORT_PLAIN_STRING)
...
_TEMPLATES: dict[str, dict[str, str]] = {
    ...
    BEACON_NEEDS_TARGET: {
        'es': 'una baliza {kind} necesita un {what} al que apuntar',
        'fr': 'une balise {kind} a besoin d\'un {what} vers quoi pointer',
        'de': 'ein {kind}-Beacon braucht ein {what}, auf das es zeigt',
        'pt': 'um farol {kind} precisa de um {what} para apontar',
        'it': 'un beacon {kind} ha bisogno di un {what} a cui puntare',
        'ja': '{kind} ビーコンには指し示す {what} が必要です',
        'zh': '{kind} 信标需要一个可指向的 {what}',
        'hi': 'एक {kind} बीकन को इंगित करने के लिए {what} चाहिए',
        'ar': 'منارة {kind} تحتاج {what} لتشير إليه',
    },
    OFFLINE_UNREACHABLE_HINT: {
        'es': 'el modo sin conexión está activado, así que {label} no puede llegar a {host}. Nada sale de esta máquina mientras PDI_OFFLINE esté activo — apúntalo a un host de esta red, o desactiva el modo sin conexión.',
        'fr': 'le mode hors ligne est activé, donc {label} ne peut pas atteindre {host}. Rien ne quitte cette machine tant que PDI_OFFLINE est actif — pointez-le vers un hôte de ce réseau, ou désactivez le mode hors ligne.',
        'de': 'der Offline-Modus ist an, daher erreicht {label} {host} nicht. Solange PDI_OFFLINE gesetzt ist, verlässt nichts diese Maschine — richte es auf einen Host in diesem Netz, oder schalte den Offline-Modus aus.',
        'pt': 'o modo offline está ligado, então {label} não pode alcançar {host}. Nada sai desta máquina enquanto PDI_OFFLINE estiver definido — aponte para um host desta rede, ou desligue o modo offline.',
        'it': 'la modalità offline è attiva, quindi {label} non può raggiungere {host}. Nulla lascia questa macchina finché PDI_OFFLINE è impostato — puntalo a un host di questa rete, o spegni la modalità offline.',
        'ja': 'オフラインモードが有効なため、{label} は {host} に到達できません。PDI_OFFLINE が設定されている間、このマシンから何も出ていきません — このネットワーク上のホストに向けるか、オフラインモードを解除してください。',
        'zh': '离线模式已开启，因此 {label} 无法访问 {host}。设置 PDI_OFFLINE 期间，任何数据都不会离开这台机器——请将其指向本网络中的主机，或关闭离线模式。',
        'hi': 'ऑफ़लाइन मोड चालू है, इसलिए {label} {host} तक नहीं पहुँच सकता। PDI_OFFLINE सेट रहते इस मशीन से कुछ बाहर नहीं जाता — इसे इसी नेटवर्क के होस्ट पर इंगित करें, या ऑफ़लाइन मोड बंद करें।',
        'ar': 'وضع عدم الاتصال مفعّل، لذا لا يستطيع {label} بلوغ {host}. لا شيء يغادر هذا الجهاز ما دام PDI_OFFLINE مضبوطًا — وجّهه إلى مضيف على هذه الشبكة، أو أوقف وضع عدم الاتصال.',
    },
    OFFLINE_UNREACHABLE: {
        'es': 'el modo sin conexión está activado, así que {label} no puede llegar a {host}. Nada sale de esta máquina mientras PDI_OFFLINE esté activo.',
        'fr': 'le mode hors ligne est activé, donc {label} ne peut pas atteindre {host}. Rien ne quitte cette machine tant que PDI_OFFLINE est actif.',
        'de': 'der Offline-Modus ist an, daher erreicht {label} {host} nicht. Solange PDI_OFFLINE gesetzt ist, verlässt nichts diese Maschine.',
        'pt': 'o modo offline está ligado, então {label} não pode alcançar {host}. Nada sai desta máquina enquanto PDI_OFFLINE estiver definido.',
        'it': 'la modalità offline è attiva, quindi {label} non può raggiungere {host}. Nulla lascia questa macchina finché PDI_OFFLINE è impostato.',
        'ja': 'オフラインモードが有効なため、{label} は {host} に到達できません。PDI_OFFLINE が設定されている間、このマシンから何も出ていきません。',
        'zh': '离线模式已开启，因此 {label} 无法访问 {host}。设置 PDI_OFFLINE 期间，任何数据都不会离开这台机器。',
        'hi': 'ऑफ़लाइन मोड चालू है, इसलिए {label} {host} तक नहीं पहुँच सकता। PDI_OFFLINE सेट रहते इस मशीन से कुछ बाहर नहीं जाता।',
        'ar': 'وضع عدم الاتصال مفعّل، لذا لا يستطيع {label} بلوغ {host}. لا شيء يغادر هذا الجهاز ما دام PDI_OFFLINE مضبوطًا.',
    },
    OLD_KEY_SEALS_KEYRING: {
        'es': 'la clave antigua no abre la versión de clave {version}, así que esta rotación sellaría el llavero — no se cambió nada',
        'fr': 'l\'ancienne clé n\'ouvre pas la version de clé {version}, donc cette rotation scellerait le trousseau — rien n\'a été changé',
        'de': 'der alte Schlüssel öffnet Schlüsselversion {version} nicht, diese Rotation würde den Schlüsselbund also versiegeln — nichts wurde geändert',
        'pt': 'a chave antiga não abre a versão de chave {version}, então esta rotação selaria o chaveiro — nada foi alterado',
        'it': 'la vecchia chiave non apre la versione di chiave {version}, quindi questa rotazione sigillerebbe il portachiavi — non è stato cambiato nulla',
        'ja': '古い鍵では鍵バージョン {version} を開けないため、このローテーションはキーリングを封じてしまいます — 何も変更されていません',
        'zh': '旧密钥打不开密钥版本 {version}，这次轮换会把钥匙环封死——未做任何更改',
        'hi': 'पुरानी कुंजी कुंजी संस्करण {version} नहीं खोलती, इसलिए यह रोटेशन कीरिंग को सील कर देता — कुछ नहीं बदला गया',
        'ar': 'المفتاح القديم لا يفتح إصدار المفتاح {version}، فهذه المناوبة كانت ستغلق حلقة المفاتيح — لم يتغير شيء',
    },
    RETENTION_ONE_OF: {
        'es': 'retention debe ser uno de {choices} o un número positivo de días',
        'fr': 'retention doit être l\'un de {choices} ou un nombre de jours positif',
        'de': 'retention muss eines von {choices} oder eine positive Tagesanzahl sein',
        'pt': 'retention deve ser um de {choices} ou um número positivo de dias',
        'it': 'retention deve essere uno tra {choices} o un numero positivo di giorni',
        'ja': 'retention は {choices} のいずれか、または正の日数にしてください',
        'zh': 'retention 必须是 {choices} 之一，或一个正的天数',
        'hi': 'retention इनमें से एक होना चाहिए: {choices}, या धनात्मक दिनों की संख्या',
        'ar': 'retention يجب أن يكون أحد {choices} أو عدد أيام موجبًا',
    },
    UNKNOWN_COMPLIANCE_PROGRAMS: {
        'es': 'programa(s) de cumplimiento desconocido(s): {got}',
        'fr': 'programme(s) de conformité inconnu(s) : {got}',
        'de': 'unbekannte(s) Compliance-Programm(e): {got}',
        'pt': 'programa(s) de conformidade desconhecido(s): {got}',
        'it': 'programma/i di conformità sconosciuto/i: {got}',
        'ja': '不明なコンプライアンスプログラム: {got}',
        'zh': '未知的合规计划：{got}',
        'hi': 'अज्ञात अनुपालन कार्यक्रम: {got}',
        'ar': 'برنامج/برامج امتثال غير معروفة: {got}',
    },
    NOT_A_TIMEZONE: {
        'es': '{got} no es una zona horaria que este sistema conozca — usa un nombre IANA como \'Europe/Lisbon\'',
        'fr': '{got} n\'est pas un fuseau horaire connu de ce système — utilisez un nom IANA comme \'Europe/Lisbon\'',
        'de': '{got} ist keine Zeitzone, die dieses System kennt — verwende einen IANA-Namen wie \'Europe/Lisbon\'',
        'pt': '{got} não é um fuso horário que este sistema conheça — use um nome IANA como \'Europe/Lisbon\'',
        'it': '{got} non è un fuso orario noto a questo sistema — usa un nome IANA come \'Europe/Lisbon\'',
        'ja': '{got} はこのシステムが知るタイムゾーンではありません — \'Europe/Lisbon\' のような IANA 名を使ってください',
        'zh': '{got} 不是本系统认识的时区——请使用 IANA 名称，如 \'Europe/Lisbon\'',
        'hi': '{got} ऐसा समय क्षेत्र नहीं जिसे यह सिस्टम जानता हो — \'Europe/Lisbon\' जैसा IANA नाम उपयोग करें',
        'ar': '{got} ليست منطقة زمنية يعرفها هذا النظام — استخدم اسم IANA مثل \'Europe/Lisbon\'',
    },
    OP_IDENTIFIER_SEGMENT: {
        'es': '\'op\' contiene un segmento que parece un identificador ({got}) — la redacción del cliente ha dejado de funcionar, y aceptarlo lo ocultaría',
        'fr': '\'op\' contient un segment qui ressemble à un identifiant ({got}) — la rédaction côté client ne fonctionne plus, et l\'accepter le masquerait',
        'de': '\'op\' enthält ein Segment, das wie ein Bezeichner aussieht ({got}) — die Schwärzung des Clients funktioniert nicht mehr, und es anzunehmen würde das verbergen',
        'pt': '\'op\' contém um segmento que parece um identificador ({got}) — a redação do cliente deixou de funcionar, e aceitá-lo esconderia isso',
        'it': '\'op\' contiene un segmento che sembra un identificatore ({got}) — la redazione del client ha smesso di funzionare, e accettarlo lo nasconderebbe',
        'ja': '\'op\' に識別子のようなセグメントが含まれています（{got}）— クライアントの伏せ字処理が機能しておらず、受け入れるとそれが隠れてしまいます',
        'zh': '\'op\' 包含一个看似标识符的片段（{got}）— 客户端的脱敏已失效，接受它会掩盖这一点',
        'hi': '\'op\' में एक खंड है जो पहचानकर्ता जैसा दिखता है ({got}) — क्लाइंट की गोपन प्रक्रिया काम नहीं कर रही, और इसे स्वीकारना उसे छिपा देगा',
        'ar': '\'op\' يحتوي على مقطع يبدو كمعرّف ({got}) — إخفاء البيانات لدى العميل توقف عن العمل، وقبوله سيخفي ذلك',
    },
    OP_SHAPE: {
    ...
    UNKNOWN_KEYS: {
        'es': 'claves de primer nivel desconocidas: {keys} — esta entrada acepta exactamente {accepts}. Una clave nueva se rechaza en vez de ignorarse: descartarla en silencio dejaría al cliente enviando contenido sin enterarse nunca de que nadie lo quería.',
        'fr': 'clés de premier niveau inconnues : {keys} — cette entrée accepte exactement {accepts}. Une nouvelle clé est refusée plutôt qu\'ignorée : la supprimer en silence laisserait le client envoyer du contenu sans jamais apprendre que personne n\'en voulait.',
        'de': 'unbekannte Schlüssel auf oberster Ebene: {keys} — diese Annahme akzeptiert genau {accepts}. Ein neuer Schlüssel wird abgelehnt statt ignoriert: ihn still zu verwerfen ließe einen Client Inhalte senden, ohne je zu erfahren, dass sie niemand wollte.',
        'pt': 'chaves de nível superior desconhecidas: {keys} — esta entrada aceita exatamente {accepts}. Uma chave nova é recusada em vez de ignorada: descartá-la em silêncio deixaria o cliente a enviar conteúdo sem nunca saber que ninguém o queria.',
        'it': 'chiavi di primo livello sconosciute: {keys} — questo intake accetta esattamente {accepts}. Una chiave nuova viene rifiutata anziché ignorata: scartarla in silenzio lascerebbe il client a inviare contenuti senza mai sapere che nessuno li voleva.',
        'ja': '不明なトップレベルキー: {keys} — この受け口が受け付けるのは {accepts} だけです。新しいキーは無視ではなく拒否されます。黙って捨てると、誰も求めていない内容をクライアントが送り続け、それを知る機会が失われます。',
        'zh': '未知的顶层键：{keys} — 此入口只接受 {accepts}。新键会被拒绝而不是被忽略：静默丢弃会让客户端持续发送没人需要的内容，且永远不会得知。',
        'hi': 'अज्ञात शीर्ष-स्तरीय कुंजियाँ: {keys} — यह इनटेक केवल {accepts} स्वीकार करता है। नई कुंजी को अनदेखा करने के बजाय अस्वीकार किया जाता है: चुपचाप हटाने से क्लाइंट सामग्री भेजता रहेगा और कभी नहीं जान पाएगा कि वह किसी को नहीं चाहिए थी।',
        'ar': 'مفاتيح عليا غير معروفة: {keys} — هذا المدخل يقبل بالضبط {accepts}. المفتاح الجديد يُرفض بدل تجاهله: إسقاطه بصمت سيترك العميل يرسل محتوى دون أن يعلم أبدًا أن أحدًا لم يرده.',
    },
    UNREADABLE_TIME: {
    ...
    SHORT_PLAIN_STRING: {
        'es': '{field} debe ser una cadena corta y simple (≤{max} caracteres, sin saltos de línea ni signos más allá de ._-()/;:+) — se recibió {got}',
        'fr': '{field} doit être une chaîne courte et simple (≤{max} caractères, sans retour à la ligne ni ponctuation au-delà de ._-()/;:+) — reçu {got}',
        'de': '{field} muss eine kurze einfache Zeichenkette sein (≤{max} Zeichen, keine Zeilenumbrüche, keine Zeichen außer ._-()/;:+) — erhalten: {got}',
        'pt': '{field} deve ser um texto curto e simples (≤{max} caracteres, sem quebras de linha nem pontuação além de ._-()/;:+) — recebido {got}',
        'it': '{field} deve essere una stringa breve e semplice (≤{max} caratteri, senza a capo né punteggiatura oltre ._-()/;:+) — ricevuto {got}',
        'ja': '{field} は短い単純な文字列にしてください（{max} 文字以内、改行や ._-()/;:+ 以外の記号は不可）— 受け取った値: {got}',
        'zh': '{field} 必须是简短的纯字符串（≤{max} 个字符，不含换行，标点仅限 ._-()/;:+）— 收到 {got}',
        'hi': '{field} एक छोटी सादी स्ट्रिंग होनी चाहिए (≤{max} अक्षर, कोई नई पंक्ति नहीं, विराम केवल ._-()/;:+) — मिला {got}',
        'ar': '{field} يجب أن يكون نصًا قصيرًا بسيطًا (≤{max} حرفًا، دون أسطر جديدة أو علامات غير ._-()/;:+) — الوارد {got}',
    },
    VAULT_NO_EARS: {
        'es': 'la bóveda no tiene oídos para esto: {detail}',
        'fr': "le coffre n'a pas d'oreilles pour ceci : {detail}",
        'de': 'der Tresor hat dafür keine Ohren: {detail}',
        'pt': 'a cripta não tem ouvidos para isto: {detail}',
        'it': 'il caveau non ha orecchie per questo: {detail}',
        'ja': '金庫にはこれを聞く耳がありません: {detail}',
        'zh': '保险库没有听这个的耳朵：{detail}',
        'hi': 'तिजोरी के पास इसके लिए कान नहीं: {detail}',
        'ar': 'الخزنة لا آذان لها لهذا: {detail}',
    },
    FETCH_READS_PAGES: {
        'es': '{tool} lee páginas, y esa url nombra una grabación — fetch.listen es la puerta que la oye',
        'fr': "{tool} lit des pages, et cette url désigne un enregistrement — fetch.listen est la porte qui l'entend",
        'de': '{tool} liest Seiten, und diese URL benennt eine Aufnahme — fetch.listen ist die Tür, die sie hört',
        'pt': '{tool} lê páginas, e essa url nomeia uma gravação — fetch.listen é a porta que a ouve',
        'it': "{tool} legge pagine, e quell'url indica una registrazione — fetch.listen è la porta che la sente",
        'ja': '{tool} はページを読むもので、その URL は録音を指しています — それを聴く扉は fetch.listen です',
        'zh': '{tool} 读取的是页面，而该 url 指向一段录音——fetch.listen 才是听它的门',
        'hi': '{tool} पृष्ठ पढ़ता है, और वह url एक रिकॉर्डिंग बताता है — fetch.listen वह द्वार है जो उसे सुनता है',
        'ar': '{tool} يقرأ الصفحات، وهذا الرابط يسمّي تسجيلًا — fetch.listen هو الباب الذي يسمعه',
    },
}
...
_REFUSALS: dict[str, dict[str, str]] = {
    ...
    "no public address to visit — reconnect with the account's handle": {
        'es': 'no hay dirección pública que visitar — vuelva a conectar con el identificador de la cuenta',
        'fr': "aucune adresse publique à visiter — reconnectez-vous avec l'identifiant du compte",
        'de': 'keine öffentliche Adresse zum Aufrufen — verbinden Sie sich erneut mit dem Kennzeichen des Kontos',
        'pt': 'não há endereço público para visitar — volte a ligar com o identificador da conta',
        'it': "nessun indirizzo pubblico da visitare — si ricolleghi con l'identificativo dell'account",
        'ja': '訪問できる公開アドレスがありません — アカウントのハンドルで接続し直してください',
        'zh': '没有可访问的公开地址 — 请用账户的标识重新连接',
        'hi': 'देखने के लिए कोई सार्वजनिक पता नहीं है — खाते के हैंडल से दोबारा जोड़ें',
        'ar': 'لا يوجد عنوان عام لزيارته — أعد الاتصال باستخدام معرّف الحساب',
    },
    'this tenant already has a customer-managed key; release it before adopting another': {
        'es': 'este inquilino ya tiene una clave gestionada por el cliente; libérela antes de adoptar otra',
        'fr': "ce locataire possède déjà une clé gérée par le client ; libérez-la avant d'en adopter une autre",
        'de': 'dieser Mandant hat bereits einen kundenverwalteten Schlüssel; geben Sie ihn frei, bevor Sie einen anderen übernehmen',
        'pt': 'este inquilino já tem uma chave gerida pelo cliente; liberte-a antes de adotar outra',
        'it': "questo tenant ha già una chiave gestita dal cliente; la rilasci prima di adottarne un'altra",
        'ja': 'このテナントにはすでに顧客管理キーがあります。別のものを採用する前に解放してください',
        'zh': '该租户已有客户自管密钥；请先释放它，再采用另一个',
        'hi': 'इस टेनैंट के पास पहले से ग्राहक-प्रबंधित कुंजी है; दूसरी अपनाने से पहले उसे मुक्त करें',
        'ar': 'لدى هذا المستأجر بالفعل مفتاح يديره العميل؛ حرّره قبل اعتماد مفتاح آخر',
    },
    'this tenant is already under deployment custody': {
        'es': 'este inquilino ya está bajo la custodia del despliegue',
        'fr': 'ce locataire est déjà sous la garde du déploiement',
        'de': 'dieser Mandant steht bereits unter der Verwahrung der Bereitstellung',
        'pt': 'este inquilino já está sob a custódia da implementação',
        'it': 'questo tenant è già sotto la custodia del deployment',
        'ja': 'このテナントはすでにデプロイメントの管理下にあります',
        'zh': '该租户已处于部署托管之下',
        'hi': 'यह टेनैंट पहले से ही परिनियोजन की अभिरक्षा में है',
        'ar': 'هذا المستأجر يخضع بالفعل لعهدة النشر',
    },
    "'count' must be a positive integer": {
        ...
        'pt': '\'count\' deve ser um inteiro positivo',
        'it': '\'count\' deve essere un intero positivo',
        'ja': '\'count\' は正の整数にしてください',
        'zh': '\'count\' 必须是正整数',
        'hi': '\'count\' एक धनात्मक पूर्णांक होना चाहिए',
        'ar': '\'count\' يجب أن يكون عددًا صحيحًا موجبًا',
    },
    "'day' must be YYYY-MM-DD": {
        'es': '\'day\' debe ser YYYY-MM-DD',
        'fr': '\'day\' doit être YYYY-MM-DD',
        'de': '\'day\' muss YYYY-MM-DD sein',
        'pt': '\'day\' deve ser YYYY-MM-DD',
        'it': '\'day\' deve essere YYYY-MM-DD',
        'ja': '\'day\' は YYYY-MM-DD 形式にしてください',
        'zh': '\'day\' 必须为 YYYY-MM-DD',
        'hi': '\'day\' YYYY-MM-DD होना चाहिए',
        'ar': '\'day\' يجب أن يكون بصيغة YYYY-MM-DD',
    },
    "'fingerprint' must be 8 hex characters": {
        'es': '\'fingerprint\' debe tener 8 caracteres hexadecimales',
        'fr': '\'fingerprint\' doit faire 8 caractères hexadécimaux',
        'de': '\'fingerprint\' muss 8 Hex-Zeichen haben',
        'pt': '\'fingerprint\' deve ter 8 caracteres hexadecimais',
        'it': '\'fingerprint\' deve essere di 8 caratteri esadecimali',
        'ja': '\'fingerprint\' は16進数8文字にしてください',
        'zh': '\'fingerprint\' 必须是 8 个十六进制字符',
        'hi': '\'fingerprint\' 8 हेक्स अक्षरों का होना चाहिए',
        'ar': '\'fingerprint\' يجب أن يكون 8 خانات سداسية عشرية',
    },
    "'op' must be a string": {
        'es': '\'op\' debe ser una cadena',
        'fr': '\'op\' doit être une chaîne',
        'de': '\'op\' muss eine Zeichenkette sein',
        'pt': '\'op\' deve ser uma string',
        'it': '\'op\' deve essere una stringa',
        'ja': '\'op\' は文字列にしてください',
        'zh': '\'op\' 必须是字符串',
        'hi': '\'op\' एक स्ट्रिंग होनी चाहिए',
        'ar': '\'op\' يجب أن يكون سلسلة نصية',
    },
    "'problems' must be a non-empty list": {
        ...
        'pt': '\'problems\' deve ser uma lista não vazia',
        'it': '\'problems\' deve essere una lista non vuota',
        'ja': '\'problems\' は空でないリストにしてください',
        'zh': '\'problems\' 必须是非空列表',
        'hi': '\'problems\' एक गैर-रिक्त सूची होनी चाहिए',
        'ar': '\'problems\' يجب أن تكون قائمة غير فارغة',
    },
    "'status' must be an HTTP status code (0 for no answer)": {
        'es': '\'status\' debe ser un código de estado HTTP (0 si no hubo respuesta)',
        'fr': '\'status\' doit être un code d\'état HTTP (0 pour aucune réponse)',
        'de': '\'status\' muss ein HTTP-Statuscode sein (0 für keine Antwort)',
        'pt': '\'status\' deve ser um código de estado HTTP (0 para sem resposta)',
        'it': '\'status\' deve essere un codice di stato HTTP (0 per nessuna risposta)',
        'ja': '\'status\' は HTTP ステータスコードにしてください（応答なしは 0）',
        'zh': '\'status\' 必须是 HTTP 状态码（无应答为 0）',
        'hi': '\'status\' एक HTTP स्थिति कोड होना चाहिए (कोई उत्तर न होने पर 0)',
        'ar': '\'status\' يجب أن يكون رمز حالة HTTP (و0 لعدم وجود إجابة)',
    },
    "an error report is an object": {
        ...
        'pt': 'um relatório de erro é um objeto',
        'it': 'una segnalazione di errore è un oggetto',
        'ja': 'エラーレポートはオブジェクトです',
        'zh': '错误报告是一个对象',
        'hi': 'एक त्रुटि रिपोर्ट एक ऑब्जेक्ट है',
        'ar': 'تقرير الخطأ كائن',
    },
    "each problem is an object": {
        ...
        'pt': 'cada problema é um objeto',
        'it': 'ogni problema è un oggetto',
        'ja': '各問題はオブジェクトです',
        'zh': '每个问题都是一个对象',
        'hi': 'प्रत्येक समस्या एक ऑब्जेक्ट है',
        'ar': 'كل مشكلة كائن',
    },
    "this deployment is reachable beyond localhost but has no PDI_ADMIN_TOKEN set — admin endpoints stay closed until one is configured (see docs/operations.md)": {
        'es': 'este despliegue es alcanzable más allá de localhost pero no tiene PDI_ADMIN_TOKEN configurado — los endpoints de administración permanecen cerrados hasta que se configure uno (ver docs/operations.md)',
        'fr': 'ce déploiement est joignable au-delà de localhost mais n\'a pas de PDI_ADMIN_TOKEN défini — les points d\'accès admin restent fermés tant qu\'aucun n\'est configuré (voir docs/operations.md)',
        'de': 'diese Bereitstellung ist über localhost hinaus erreichbar, hat aber kein PDI_ADMIN_TOKEN gesetzt — die Admin-Endpunkte bleiben geschlossen, bis eines konfiguriert ist (siehe docs/operations.md)',
        'pt': 'esta implantação é alcançável além de localhost mas não tem PDI_ADMIN_TOKEN definido — os endpoints de administração ficam fechados até um ser configurado (ver docs/operations.md)',
        'it': 'questo deployment è raggiungibile oltre localhost ma non ha PDI_ADMIN_TOKEN impostato — gli endpoint di amministrazione restano chiusi finché non ne viene configurato uno (vedi docs/operations.md)',
        'ja': 'このデプロイは localhost の外から到達できますが、PDI_ADMIN_TOKEN が設定されていません — 設定されるまで管理エンドポイントは閉じたままです（docs/operations.md 参照）',
        'zh': '此部署可从 localhost 之外访问，但未设置 PDI_ADMIN_TOKEN——在配置之前，管理端点保持关闭（见 docs/operations.md）',
        'hi': 'यह परिनियोजन localhost से आगे पहुँच योग्य है पर PDI_ADMIN_TOKEN सेट नहीं — जब तक एक कॉन्फ़िगर न हो, एडमिन एंडपॉइंट बंद रहते हैं (docs/operations.md देखें)',
        'ar': 'هذا النشر يمكن بلوغه من خارج localhost لكن دون PDI_ADMIN_TOKEN — تبقى نقاط الإدارة مغلقة حتى يُضبط واحد (انظر docs/operations.md)',
    },
    "a standing task repeats between a quarter-hour and a month": {
        'es': 'una tarea permanente se repite entre un cuarto de hora y un mes',
        'fr': "une tâche permanente se répète entre un quart d'heure et un mois",
        'de': 'eine stehende Aufgabe wiederholt sich zwischen einer Viertelstunde und einem Monat',
        'pt': 'uma tarefa permanente repete-se entre um quarto de hora e um mês',
        'it': "un compito permanente si ripete tra un quarto d'ora e un mese",
        'ja': '定期タスクの繰り返し間隔は15分から1か月の範囲です',
        'zh': '常设任务的重复间隔在一刻钟到一个月之间',
        'hi': 'एक स्थायी कार्य सवा घंटे से एक महीने के बीच दोहराता है',
        'ar': 'المهمة الدائمة تتكرر بين ربع ساعة وشهر',
    },
    "a standing task repeats on a number of hours": {
        'es': 'una tarea permanente se repite en un número de horas',
        'fr': "une tâche permanente se répète sur un nombre d'heures",
        'de': 'eine stehende Aufgabe wiederholt sich in einer Anzahl von Stunden',
        'pt': 'uma tarefa permanente repete-se num número de horas',
        'it': 'un compito permanente si ripete su un numero di ore',
        'ja': '定期タスクの繰り返しは時間数で指定します',
        'zh': '常设任务按小时数重复',
        'hi': 'एक स्थायी कार्य घंटों की संख्या पर दोहराता है',
        'ar': 'المهمة الدائمة تتكرر على عدد من الساعات',
    },
    "fetch.listen needs an http(s) url": {
        'es': 'fetch.listen necesita una url http(s)',
        'fr': "fetch.listen a besoin d'une url http(s)",
        'de': 'fetch.listen braucht eine http(s)-URL',
        'pt': 'fetch.listen precisa de uma url http(s)',
        'it': 'fetch.listen ha bisogno di una url http(s)',
        'ja': 'fetch.listen には http(s) の URL が必要です',
        'zh': 'fetch.listen 需要 http(s) 网址',
        'hi': 'fetch.listen को एक http(s) url चाहिए',
        'ar': 'fetch.listen يحتاج رابط http(s)',
    },
    "forgetting needs a key": {
        'es': 'olvidar necesita una clave',
        'fr': 'oublier demande une clé',
        'de': 'Vergessen braucht einen Schlüssel',
        'pt': 'esquecer precisa de uma chave',
        'it': 'dimenticare richiede una chiave',
        'ja': '忘却には鍵が必要です',
        'zh': '遗忘需要一个键',
        'hi': 'भूलने के लिए एक कुंजी चाहिए',
        'ar': 'النسيان يحتاج مفتاحًا',
    },
    "say something to ask — an empty prompt generates nothing": {
        'es': 'di algo que preguntar — un prompt vacío no genera nada',
        'fr': 'dites quelque chose à demander — un prompt vide ne génère rien',
        'de': 'sag etwas zu fragen — ein leerer Prompt erzeugt nichts',
        'pt': 'diga algo a perguntar — um prompt vazio não gera nada',
        'it': "di' qualcosa da chiedere — un prompt vuoto non genera nulla",
        'ja': '何か尋ねる内容を書いてください — 空のプロンプトからは何も生成されません',
        'zh': '说点要问的——空提示什么也生成不了',
        'hi': 'पूछने को कुछ कहें — खाली प्रॉम्प्ट कुछ नहीं बनाता',
        'ar': 'قل شيئًا تسأله — الموجّه الفارغ لا يولّد شيئًا',
    },
    "the wire is down": {
        'es': 'el cable está caído',
        'fr': 'le fil est coupé',
        'de': 'die Leitung ist tot',
        'pt': 'o fio caiu',
        'it': 'il filo è giù',
        'ja': '回線が落ちています',
        'zh': '线路已断',
...
_FIELD_LABELS: dict[str, dict[str, str]] = {
    ...
    'question': {'en': 'Say something to ask — it never leaves this host.', 'es': 'Escribe tu pregunta — nunca sale de este servidor.', 'fr': 'Écrivez votre question — elle ne quitte jamais cet hôte.', 'de': 'Schreib deine Frage — sie verlässt diesen Host nie.', 'pt': 'Escreva a sua pergunta — nunca sai deste servidor.', 'it': 'Scrivi la tua domanda — non lascia mai questo host.', 'ja': '質問を入力してください — このホストの外には出ません。', 'zh': '输入你的问题——它绝不会离开这台主机。', 'hi': 'अपना सवाल लिखें — यह इस होस्ट से बाहर कभी नहीं जाता।', 'ar': 'اكتب سؤالك — لا يغادر هذا المضيف أبدًا.'},
    'prefix': {'en': 'Only keys under this prefix', 'es': 'Solo claves bajo este prefijo', 'fr': 'Seules les clés sous ce préfixe', 'de': 'Nur Schlüssel unter diesem Präfix', 'pt': 'Apenas chaves sob este prefixo', 'it': 'Solo chiavi sotto questo prefisso', 'ja': 'このプレフィックス配下のキーのみ', 'zh': '仅限此前缀下的键', 'hi': 'केवल इस उपसर्ग के तहत कुंजियाँ', 'ar': 'المفاتيح تحت هذه البادئة فقط'},
    'system': {'en': 'How the answer should speak', 'es': 'Cómo debe hablar la respuesta', 'fr': 'Comment la réponse doit parler', 'de': 'Wie die Antwort sprechen soll', 'pt': 'Como a resposta deve falar', 'it': 'Come deve parlare la risposta', 'ja': '答えの話し方', 'zh': '回答的说话方式', 'hi': 'जवाब कैसे बोले', 'ar': 'كيف يجب أن تتحدث الإجابة'},
