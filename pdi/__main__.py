"""``python -m pdi`` — run the PDI console, phone-ready, in one command.

``python -m pdi phone`` does everything the README's manual steps do:
...
Security posture unchanged: the address is local-network only, admin
endpoints still require PDI_ADMIN_TOKEN, and tenant data still requires
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m pdi",
        description="Run the PDI console, phone-ready, in one command.")
