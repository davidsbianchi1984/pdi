"""Operating the PDI console from a phone.

The operator console in ``app/`` is a web app, so a phone on the same
network can run it directly from this backend — no app store, no separate
dev server. Two pieces make that work:
...
Nothing here changes the security posture: admin endpoints still require
``PDI_ADMIN_TOKEN`` and tenant data still requires the tenant's bearer
token. A phone on the LAN is exactly as authorized as a laptop on the LAN.
"""
...
def console_dir() -> Path | None:
    ...
    override = os.environ.get("PDI_CONSOLE_DIR")
...
def lan_address() -> str:
    ...
    ``PDI_LAN_HOST`` overrides everything for unusual networks.
    """
    forced = os.environ.get("PDI_LAN_HOST")
    if forced:
        return forced
...
def pairing(port: int = 8000) -> dict:
    ...
    if not reachable:
        ...
        how += ["No local-network address found for this machine — a phone "
                ...
                "Check Wi-Fi is connected, then set PDI_LAN_HOST to this "
    ...
    how = ["Put the phone on the same Wi-Fi as this machine.",
           "Start the API on all interfaces: uvicorn pdi.api:app --host 0.0.0.0",
           *how]
    return {
        ...
        "note": "Local network only — this address is not reachable from the "
                "internet, which is the point: the vault and its sealed "
                "records stay on your own network.",
    }
