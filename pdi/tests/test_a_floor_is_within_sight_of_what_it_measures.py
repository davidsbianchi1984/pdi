...
Two rows passed, and this is the product where they would. `len(made) > 20`
was set low deliberately because the three products' shells differ by a factor
of three in size, and this is the small one — twenty against thirty-five is a
real floor here and a twentieth of QRME's four hundred and thirty. One number
written to work in three repositories is calibrated for whichever of them was
smallest when it was written. Neither repository could tell, because neither
...
def test_a_decorative_floor_is_caught():
    ...
    assert not (10 >= 48 * HEADROOM)
    assert 38 >= 48 * HEADROOM
