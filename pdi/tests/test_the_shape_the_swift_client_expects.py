...
def test_no_swift_struct_promises_what_the_route_cannot_give_it(client):
    ...
    loose = []
    for _, _, findings in _drive(client, token, key):
...
def test_the_extractor_reads_the_swift_client():
    ...
    assert len(_structs()) >= 25, len(_structs())
    assert fields >= 80, fields
    assert len(_bindings()) >= 14, len(_bindings())


def test_the_scan_reaches_a_real_share_of_the_bindings(client):
    token, key = _standing(client)
    driven = [f for _, _, f in _drive(client, token, key)
              if f is not None]
    assert len(driven) >= 8, (
