...
def served(monkeypatch_session=None):
    ...
    before = os.environ.get("PDI_CORS_ORIGINS")
    console_before = os.environ.get("PDI_CONSOLE_DIR")
    os.environ["PDI_CORS_ORIGINS"] = "*"
    os.environ["PDI_CONSOLE_DIR"] = str(dist)
    try:
        app = create_app()
    finally:
        for name, kept in (("PDI_CORS_ORIGINS", before),
                           ("PDI_CONSOLE_DIR", console_before)):
            if kept is None:
...
STRANGER_PAGES = (("/s/no-such-card", 404),
                  ("/r/no-such-thread", 200))
...
def test_a_reflected_parameter_is_not_markup(served):
    ...
    """
    port, _ = served
    pytest.skip("this product has no sign-in provider callback; the "
                "escaping rule is carried by the sweep instead")
