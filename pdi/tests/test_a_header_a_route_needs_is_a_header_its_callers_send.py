...
`_tenant` reads `x-tenant-key`, and `_tenant_kek` refuses without it. Driven
over the API, after pressing *hold our own key* on the Custody screen:
...
The console sent the header on `/bequests/grant/keys` and
`/bequests/grant/read`, which are the heir's routes, and on nothing else. The
three shells sent it nowhere. So the Custody screen shipped a button that
locked every client in the product out of every record, and its own *hand it
back* button — the one thing that undoes it — was behind the same refusal.

That is not a parity failure. Both sides of the comparison were wrong in the
same direction, which is exactly the case a comparison cannot report.
...
def test_the_key_this_product_asks_for_is_sendable_by_every_client():
    ...
    assert not blind, (
        ...
        f"which this product requires to carry a customer-managed key. A client that can "
