...
reader's language, through three handlers that all return by one door. It missed
...
`RequestValidationError` is neither an `HTTPException` nor a domain error. FastAPI raises it before
...
handed it straight back — a record value in plaintext, on the one path in an
...
def test_the_handler_is_registered(client):
    ...
    refused = client.put("/records", json={"k": "x"},
                         headers=auth(new_tenant(client)))
...
def _body_routes(app):
    ...
    for route in all_routes(app):
        ...
        if not methods:
            continue
        path = re.sub(r"\{[^}]+\}", "tnt_000000000000", route.path)
        if "{" in path:
            continue
...
def _sweep(client):
    ...
    With a **real** tenant token, and that is not a convenience. `_tenant` is a
    dependency, and a dependency that raises aborts the request before the
    body model is ever validated — so an unauthenticated sweep answers 401
    forty times and reports a spotless vault it never asked. Twelve routes
    reached validation without a token; thirty-one reach it with one.
    """
    from .conftest import auth, new_tenant
    head = auth(new_tenant(client, "canary"))
    leaked, reached = [], 0
...
def test_the_message_is_in_the_readers_language(client):
    ...
    Through the tenant's stored setting, not the header — this vault refuses
    an unauthenticated write before validation runs, so there is no such thing
    here as a 422 from a caller with no credential. The header is sent anyway,
    saying English, because that is what a console operator's browser sends
    whatever they chose in the app.
    """
    from .conftest import auth, new_tenant
    head = auth(new_tenant(client))
