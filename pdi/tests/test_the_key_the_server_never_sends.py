...
REPO = _repo_root()
PKG = "pdi"
ANDROID = "native/android/app/src/main/java/com/pdi/vault"
...
#: here: PDI is the bottom of the stack — it proxies nobody.
PROXIED: set[str] = set()

#: (label, the one file that decodes bodies, the floor of distinct keys)
SHELLS = (
    ("ios", "native/ios/Sources/ApiClient.swift", 50),
    ("android", f"{ANDROID}/ApiClient.kt", 48),
    ("windows", "native/windows/ApiClient.cs", 48),
)
