...
def test_the_extractors_are_reading_something():
    ...
    android = list((REPO / "native/android").rglob("ApiClient.kt"))
    if android:
        counts["android"] = (android[0], r"^\s{4}(?:suspend )?fun (\w+)\(")

    for name, (path, pattern) in counts.items():
        if not path.exists():
            continue
        n = len(re.findall(pattern, path.read_text(encoding="utf-8"), re.M))
        assert n > 20, (
            f"only {n} {name} bindings found — the pattern has stopped "
