...
a sibling product read "this build has no collector configured, so nothing
...
the gateway (`pdi/problems.py`), folded into counters, readable by the
...
def _report(**over):
    base = {
        "source": "pdi", "app_version": "0.68.0", "platform": "iPhone",
        "language": "en",
        "problems": [{"op": "POST /intake/{id}/submit", "status": 500,
...
def test_a_report_lands_and_folds(client):
    ...
    assert rows[0]["op"] == "POST /intake/{id}/submit"
...
def test_the_intake_is_a_whitelist(client):
    ...
    r = client.post("/v1/problems", json=_report(problems=[{
        "op": "GET /intake/itk_8f2k1x9q/status", "status": 404,
...
def test_reading_is_narrower_than_writing(client, monkeypatch):
    ...
    monkeypatch.setenv("PDI_PROBLEMS_KEY", "letmesee")
    client.post("/v1/problems", json=_report())
    ...
    assert ok.json()["rows"][0]["source"] == "pdi"
