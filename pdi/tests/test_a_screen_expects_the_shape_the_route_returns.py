...
`GET /hosting/{tenant_id}/history` returns an **object**:
...
That is not a wrong render, it is a thrown exception during render, so the
whole *where this vault has lived* card goes — and it goes only on a vault
that has been moved at least once, which no fresh test vault ever has.

JIM-mini had the same defect on `GET /users/{uid}/referral/clinicians`, and
worse: the object it returns carries a `reason` for an empty list — *no
clinician registered* — that nobody had ever seen, because the screen threw
before reaching it.
