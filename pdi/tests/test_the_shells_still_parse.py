...
not C#.** They are XAML — 775 lines of it here, more than every `.cs` file
...
Five pages across two products do not parse. None of the five are here — this shell is the one that happened to be right, which is worth knowing rather than assuming. Each one is a
...
def test_there_are_shell_sources_to_read():
    ...
    assert len(SWIFT) >= 15, len(SWIFT)
    assert len(KOTLIN) >= 6, len(KOTLIN)
    assert len(CSHARP) >= 13, len(CSHARP)
