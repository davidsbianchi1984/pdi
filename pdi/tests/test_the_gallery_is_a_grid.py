...
was clipped past the fourth. QRME's main gallery was worse — one `<tr>` carrying
**fifteen** cells beside rows of three, which made that table fifteen columns
wide and left twelve empty columns on almost every row. This repo's own lists
split four pictures as three-then-one, which is the same defect in miniature.
...
#: reading them, so the same shape rule on both.
