"""Running the operator console from a phone: pairing details and the served console."""
...
def test_pairing_says_so_when_no_reachable_address_exists(client, monkeypatch):
    ...
    monkeypatch.setenv("PDI_LAN_HOST", "127.0.0.1")
    body = client.get("/pair").json()
    assert body["reachable"] is False
    assert any("cannot reach 127.0.0.1" in step for step in body["how"])
    assert any("PDI_LAN_HOST" in step for step in body["how"])
...
def test_pairing_reports_every_blocker_at_once(client, monkeypatch):
    ...
    monkeypatch.setenv("PDI_CONSOLE_DIR", "/nonexistent/dist")
    monkeypatch.setenv("PDI_LAN_HOST", "127.0.0.1")
...
def test_console_is_served_when_built(tmp_path, monkeypatch):
    ...
    dist = tmp_path / "dist"
    dist.mkdir()
    (dist / "index.html").write_text("<!doctype html><title>PDI Console</title>")
    (dist / "manifest.webmanifest").write_text('{"name":"PDI Console"}')
    ...
    with TestClient(create_app()) as c:
        ...
        assert "PDI Console" in index.text
...
def test_bare_invocation_offers_every_way_to_run(capsys):
    """`python -m pdi` with no arguments is the launcher menu: phone,
    desktop, packaged installer, and headless API — the user picks the
    device; every option is one command."""
    from pdi.__main__ import main
    assert main([]) == 0
    out = capsys.readouterr().out
    assert "python -m pdi phone" in out
    assert "python -m pdi desktop" in out
    assert "python -m pdi serve" in out
