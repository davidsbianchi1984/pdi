...
The finding and the guard are the sibling product's. This repo's three tables
hold 51, 64 and 58 rows and no English string appears under two keys in any of
them, so `native_split_wordings.txt` here is empty and is a floor.

That is not discipline. This shell's tables are small because most of its
screens are still English — `native_screens_untranslated.txt` records 65, 59
and 69 — and a table that holds few sentences cannot hold one twice. The empty
record is the *before* picture, and the guard is here first so that the rows
still owed arrive checked rather than audited two releases later, which is what
happened in the sibling repo and is the reason this file exists at all.
