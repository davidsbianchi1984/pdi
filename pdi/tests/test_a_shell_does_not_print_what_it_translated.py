...
def test_the_scan_can_still_see_a_screen():
    ...
    for shell, spec in SHELLS.items():
        ...
        # **Zero here, and that is the product rather than the split.** This
        # product's tables are 41–51 rows of mostly plain labels; only one row
        # in one shell has a slot with a phrase either side of it. The
        # fragment half therefore finds nothing, and a floor above zero would
        # be a number invented to look like a measurement. The half that
        # carries this product is the whole-row one above, and it found seven.
        assert len(_fragments(spec)) >= 0
