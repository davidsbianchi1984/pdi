...
SHELLS = {
    ...
    "android": {
        "table": "native/android/app/src/main/java/com/pdi/vault/L10n.kt",
        "views": "native/android/app/src/main/java/com/pdi/vault/ui",
        "exts": (".kt",),
...
#: Two SwiftUI enum raw values, both of them the stored identity of a
#: selected tab and both with a `key` beside them that reads the row:
#: `SourcesView.Tab.connectors` and `TransfersView.Direction.outbound`.
#:
#: The `SourcesView` one is here because 0.57.8 gave it that `key`. The picker
#: rendered `$0.rawValue` directly, so both of its tabs read English on every
#: device — the rule `TransfersView` had settled next door, never applied one
#: file over. Renaming a raw value changes what a restored selection means and
#: changes nothing a reader sees, so the raw values stay.
RECORDED = {
    ('ios', 'Connectors'),
    ('ios', 'Outbound'),
}
...
def test_the_scan_can_still_see_a_screen():
    ...
    for shell, spec in SHELLS.items():
        ...
        # This line read `>= 0` under a comment saying the fragment half
        # finds nothing here, so a floor above zero would be a number
        # invented to look like a measurement. The premise was wrong: the
        # split finds seven fragments on the phones and nine on the desktop.
        #
        #     asked     is a floor of zero honest about a small surface
        #     mattered  is the surface actually zero
        #
        # It was an assertion that could not fail, which is the same thing as
        # no assertion. Measured and registered per shell like its sibling.
        assert len(_fragments(spec)) >= ratchets.floor(
                f"shell.fragments.{shell}"), (
            f"{shell}: only {len(_fragments(spec))} fragment(s) out of its "
            f"slotted rows — the split has stopped finding slots")
