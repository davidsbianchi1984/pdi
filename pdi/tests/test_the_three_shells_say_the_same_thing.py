...
Nothing. 51 keys held by two or more shells, 47 English strings held by all
three, and no disagreement in either set.

That is worth as little celebration as this repo's empty
`native_split_wordings.txt`: these tables are small because most of this
product's screens are still English, and three short tables written together
in one round have had little chance to drift. The guard is in place for when
they grow, which `console_untranslated.txt` says is 250 strings away.
...
FLOOR, PROBE, OVERLAP = 45, "nfil.content", 40
