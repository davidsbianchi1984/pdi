...
struct ProblemReportingCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text(L10n.t("prb.title", state.language)).font(.headline)
            ...
            if Problems.collectorUrl().isEmpty {
                ...
                Text(L10n.t("prb.nowhere", state.language))
                    .font(.footnote).foregroundStyle(Theme.t2)
            } else if !answered {
                Text(L10n.t("prb.can", state.language))
                    .font(.footnote).foregroundStyle(Theme.t2)
                HStack(spacing: 10) {
                    Button(L10n.t("prb.send", state.language)) { answer(true) }
                        .buttonStyle(.bordered)
                    Button(L10n.t("prb.donot", state.language)) { answer(false) }
                        .buttonStyle(.bordered)
                }
            } else {
                Toggle(isOn: Binding(get: { sending },
                    ...
                    Text(L10n.t("prb.toggle", state.language)).font(.subheadline)
                        .foregroundStyle(Theme.txt)
                }
                .tint(Theme.brandA)
            }

            Button(showing ? L10n.t("prb.hide", state.language)
                           : L10n.t("prb.show", state.language)) { showing.toggle() }
                .font(.caption).foregroundStyle(Theme.brandA)

            if showing {
                if rows.isEmpty {
                    Text(L10n.t("prb.owed.none", state.language))
