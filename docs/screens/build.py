...
SCREENS = [
    ...
    # 48-52 close the console backlog: eighty-four routes the desktop app
    # could not reach, all of them present on the phone shells.
    dict(num=48, title="Carriers", sub="A sealed thing, and its code",
         accent="cyan", tab=0, cards=[
        dict(icon="lock", color="cyan", k="Server rack A",
             s="object · sealed · blind"),
        dict(icon="eye", color="amber", k="Contents",
             s="not disclosed, on any setting"),
        dict(icon="finger", color="green", k="A courier left a note",
             s="in the chain, not over it"),
        dict(icon="shieldok", color="green", k="Chain verifies",
             s="four events, hash-linked"),
    ]),
    dict(num=49, title="Exchange", sub="Out sealed, and in by invitation",
         accent="brand", tab=0, cards=[
        dict(icon="doc", color="brand", k="scan.pdf → clinic-a",
             s="sealed · 28 bytes"),
        dict(icon="link", color="amber", k="Receive token",
             s="shown once, never again"),
        dict(icon="plus", color="cyan", k="Intake from Dr Osei",
             s="open · records request"),
        dict(icon="shield", color="green", k="Not your token",
             s="the other party holds theirs"),
    ]),
    dict(num=50, title="Custody", sub="The key, the hardware, the paperwork",
         accent="amber", tab=0, cards=[
        dict(icon="lock", color="green", k="Operator can decrypt",
             s="no — the customer holds it"),
        dict(icon="cross", color="amber", k="Reseal skipped 12",
             s="we could not open them"),
        dict(icon="doc", color="cyan", k="BAA on file",
             s="effective 1 August"),
        dict(icon="building", color="brand", k="Our facility",
             s="free, and here is why"),
    ]),
    dict(num=51, title="Bridges", sub="What reaches into this vault",
         accent="green", tab=0, cards=[
        dict(icon="chat", color="brand", k="mastodon @ops",
             s="publish · 4 out"),
        dict(icon="bolt", color="amber", k="Hall NEO",
             s="bound · 2 sent in"),
        dict(icon="db", color="cyan", k="41 contributions held",
             s="keys and a count, no contents"),
        dict(icon="shieldok", color="green", k="Sealed on arrival",
             s="under this tenant's key"),
    ]),
    dict(num=52, title="Guiding", sub="The console's own guide",
         accent="cyan", tab=0, cards=[
        dict(icon="compass", color="cyan", k="No name, no face",
             s="and it cannot read the vault"),
        dict(icon="chat", color="brand", k="Where is the audit log?",
             s="answered from a written table"),
        dict(icon="speaker", color="amber", k="No machine translation",
             s="its own notes, and it says so"),
