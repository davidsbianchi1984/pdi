...
PDI, [QRME](https://github.com/davidsbianchi1984/qrme) and [JIM-mini](https://github.com/davidsbianchi1984/jim-mini) are built to run in
...
entry reads *there are no functional changes to PDI in this release*, in those
...
2. Bump the version string in **all twelve places**. Five are the backend
   and the console: `pyproject.toml`, the `FastAPI(...)` call in `pdi/api.py`,
...
3. Tag and push:
   ...
   **Tag creation is the maintainer's step, and it needs a link.** Claude
   ...
   ```
   https://github.com/davidsbianchi1984/pdi/releases/new?tag=app-v0.19.0&target=<sha>&title=PDI%20app-v0.19.0
   ```
   ...
   **The title carries the product name**: `PDI app-v0.19.0`, not
