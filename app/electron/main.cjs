...
//
// The installer also ships the PDI backend itself — a PyInstaller
...
const DEFAULT_PORT = Number(process.env.PDI_PORT || 8000);
...
function bundledBackend() {
  const name = process.platform === "win32" ? "pdi-backend.exe" : "pdi-backend";
...
async function ensureBackend() {
  ...
  backendProc = spawn(bin, [], {
    env: {
      ...process.env,
      PDI_DB: path.join(app.getPath("userData"), "pdi.db"),
      PDI_PORT: String(backendPort),
    },
...
function createWindow() {
  const win = new BrowserWindow({
    ...
    webPreferences: {
      ...
      additionalArguments: [`--pdi-backend-url=http://127.0.0.1:${backendPort}`],
    },
  });
